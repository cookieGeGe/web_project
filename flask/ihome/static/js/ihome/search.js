...
function updateHouseData(action) {
    ...
    if (undefined == areaId) {
        if (undefined != decodeQuery()['aid']) {
            areaId = decodeQuery()['aid']
        } else {
            areaId = ''
        }
    }
    ...
    getinfo(areaId, startDate, endDate, sortKey)
    //发起ajax请求，获取数据，并显示在模板中
}

function getinfo(aid, startDate, endDate, sk) {
    $.ajax({
        url: '/index/allsearch/',
        type: 'GET',
        dataType: 'json',
        data: {'aid': aid, 'sd': startDate, 'ed': endDate, 'sk': sk},
        success: function (data) {
            if (data.code=='200'){
                if ($('.filter-area').find('li').length==0){
                    var area_temp = template('area_tem', {areas: data.alist});
                    $('.filter-area').html(area_temp);
                }


                var house_temp = template('house_temp', {houses: data.showhouse});
                $('.house-list').html(house_temp);
            }
...
$(document).ready(function () {
    ...
    getinfo(aid, startDate, endDate, sk)
    ...
    $(".filter-item-bar>.filter-sort").on("click", "li", function (e) {
        ...
        if (!$(this).hasClass("active")) {
            ...
            // alert($(this))
            // url = '/index/search/' + location.search.split('&sk=')[0] + "&sk=" + $(this).attr('sort-key')
            // location.href = url
